	...
	<div class="col-xs-12">
		<div class="box">
			...
			<div class="box-body" id="table_fill">
				...
				<tr>
					<th>No</th>
					<th>Surat Permohonan Assessing</th>
					<th>Nama, Tempat Tanggal Lahir</th>
					<th>Pihak Keluarga</th>
					<th>Identitas Jaksa dan Pengawal Tahanan</th>
					<th>Keluhan Medis</th>
				...
				<tbody>
					...
					<!-- <td>1</td>
					<td>Asssesing-01/2019/06-1</td>
					<td>User1. Jakarta, 15 Agustus 1995</td>
					<td>Test User1</td>
					<td>Jaksa1 - Pengawal1</td>							
					<td>Stroke</td>
					<td>Proses hukum1</th>
								...
								<td>
									<?php
										if ($row->status == 0) {
											# code...
									?>
										<button class="btn btn-warning btn-xs" onclick="upload('<?php echo $row->id;?>')"><i class="fa fa-upload"></i> Unggah</button>&nbsp;&nbsp;&nbsp;
									<?php
										}
										else {
											# code...
									?>
										<a href="<?php base_url();?>public/assessing/<?=$row->audit_user_insert;?>/<?=$row->berkas;?>" class="btn btn-success btn-xs"><i class="fa fa-download"></i> Unduh</a>									
									<?php											
										}
									?>
								</td>						
								<td>
									<?=$row->audit_time_insert;?>
									<?php
										if ($this->session->userdata('_session_role') == 4) 
										{
											if ($row->status == 0) {
												# code...
									?>
												<label>Harap Unggah Berkas yang dibutuhkan (Surat Permohonan)</label>
									<?php
											}
										}									
									?>
									...
									<?php
										if ($this->session->userdata('_session_role') == 4) 
										{
											if ($row->status < 2) {
												# code...
									?>
												<button class="btn btn-warning btn-xs" onclick="edit('<?php echo $row->id;?>')"><i class="fa fa-edit"></i> Ubah</button>&nbsp;&nbsp;&nbsp;
												<button class="btn btn-danger btn-xs" onclick="del('<?php echo $row->id;?>')"><i class="fa fa-trash"></i> Hapus</button>									
									<?php
											}
										}
										elseif($this->session->userdata('_session_role') == 3)
										{
											if ($row->status == 1) {
												# code...
									?>
												<button class="btn btn-primary btn-xs col-lg-12" style="margin-bottom:10px;" onclick="proses('<?php echo $row->id;?>')"><i class="fa fa-edit"></i> Pengajuan Ke Direktur</button>														
									<?php												
											}
										}
										elseif($this->session->userdata('_session_role') == 5)
										{
											if ($row->status == 2) {
												# code...
									?>
												<button class="btn btn-primary btn-xs col-lg-12" style="margin-bottom:10px;" onclick="proses_direktur('<?php echo $row->id;?>')"><i class="fa fa-edit"></i> Permintaan Pelayanan</button>														
									<?php												
											}
										}										
